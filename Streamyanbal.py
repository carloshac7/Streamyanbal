...
# === LECTURA DEL ARCHIVO DESDE SHAREPOINT ===
@st.cache_data
def cargar_datos_sharepoint():
    ...
    try:
        ...
        download_url = sharepoint_url.replace('/:x:/', '/_layouts/15/download.aspx?SourceUrl=')
        download_url = download_url.split('?')[0]  # Quitar parámetros extra
        
        # Opción alternativa: URL directa (puede requerir ajustes según la configuración de SharePoint)
        # Si el método anterior no funciona, intenta con esta URL modificada:
        base_url = "https://uniqueyanbal.sharepoint.com/sites/DataAnalytics941/_layouts/15/download.aspx"
        doc_id = "ERCSfpouz4VGq9aEmpg9hu0B7FEwE-kHOBeK7aIwzmIaPw"
        direct_url = f"{base_url}?docid={doc_id}&authkey=!AQHsBCz68gUE3_U"
        ...
        if response.status_code == 200:
            ...
            # Si falla, intentar con el enlace público si está disponible
            # Nota: El archivo debe estar compartido públicamente para esto
            public_url = "https://uniqueyanbal.sharepoint.com/:x:/s/DataAnalytics941/ERCSfpouz4VGq9aEmpg9hu0B7FEwE-kHOBeK7aIwzmIaPw?e=jAWV27&download=1"
            df = pd.read_excel(public_url)
            return df, None
...
with st.spinner('Cargando datos desde SharePoint...'):
    df, error = cargar_datos_sharepoint()

if error:
    st.error(f"❌ {error}")
    st.info("💡 Alternativas para solucionar el problema:")
    st.markdown("""
    1. **Verificar permisos**: Asegúrate de que el archivo esté compartido públicamente
    2. **Usar archivo local**: Sube el archivo directamente en Streamlit
    3. **URL alternativa**: Intenta generar un enlace de descarga directa desde SharePoint
    """)
    
    # Opción para cargar archivo localmente como respaldo
    st.markdown("---")
    st.subheader("📁 Cargar archivo manualmente")
...
st.sidebar.markdown("---")
st.sidebar.caption(f"🔄 Datos cargados desde SharePoint")
st.sidebar.caption(f"📅 Última sincronización: Ahora")
...
# Numerar las ejecuciones de la misma base de datos en el mismo periodo
df_filtrado['num_ejecucion'] = df_filtrado.groupby('Nombre Modelo Semántico').cumcount() + 1

# Identificador con número de ejecución si hay duplicados
...
# Para ejecuciones múltiples de la misma base de datos, asignar colores diferentes
...
# Mapeo de colores
color_map = {
    'MAM - 1ra ejecución': '#1f77b4',    # Azul oscuro
    'MAM - 2da ejecución': '#6baed6',    # Azul claro
    'MAM - 3ra+ ejecución': '#c6dbef',   # Azul muy claro
    'MAC - 1ra ejecución': '#ff7f0e',    # Naranja oscuro
    'MAC - 2da ejecución': '#ffbb78',    # Naranja claro
    'MAC - 3ra+ ejecución': '#ffd699'    # Naranja muy claro
}
